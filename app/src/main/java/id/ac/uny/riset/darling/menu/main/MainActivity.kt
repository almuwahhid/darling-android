...
import id.ac.uny.riset.darling.Service.AlarmReceiver
import id.ac.uny.riset.darling.Service.CheckIntervensiJobScheduler
...
import id.ac.uny.riset.darling.menu.about.AboutActivity
import id.ac.uny.riset.darling.menu.biodata.BiodataActivity
import id.ac.uny.riset.darling.menu.login.LoginActivity
import id.ac.uny.riset.darling.menu.main.adapter.AdapterTaskPertanyaan
import id.ac.uny.riset.darling.menu.panduan.PanduanActivity
import id.ac.uny.riset.darling.menu.survey.SurveyActivity
import id.ac.uny.riset.darling.menu.surveySaya.SurveySayaActivity
...
import id.ac.uny.riset.darling.utils.avatarview.views.AvatarView
import id.ac.uny.riset.darling.utils.dialogs.DialogConfirm.DialogConfirm
import id.ac.uny.riset.darling.utils.dialogs.DialogHubungiWhatsApp.DialogCallWhatsApp
import id.ac.uny.riset.darling.utils.dialogs.DialogSubmitTaskPertanyaan.DialogSubmitTaskPertanyaan
...
class MainActivity : BaseActivity(), NavigationView.OnNavigationItemSelectedListener, MainView.View{
            ...
            override fun onPertanyaanClick(intvrl: TaskPertanyaanModel) {
                if(intvrl.status_task.equals("N")){
                    if(Function.isToday(intvrl.tanggal_task).equals("today")){
                        ...
                        LibUi.ToastShort(context, "Intervensi sudah berlalu, Anda tidak dapat melakukannya kembali")
                    } else if(Function.isToday(intvrl.tanggal_task).equals("tomorrow")){
                        LibUi.ToastShort(context, "Belum masuk waktu mengerjakan intervensi")
                    }
                } else {
                    LibUi.ToastShort(context, "Intervensi sudah dikerjakan, Terima kasih")
                }
            }
        })
    ...
    override fun onResume() {
        ...
        (navView.findViewById(R.id.tv_status) as TextView).setText(Function.getUserPreference(context).nama_status)
    ...
    override fun onDoSurvey(surveyModel: SurveyModel, taskIntervensiModels: MutableList<TaskPertanyaanModel>) {
        ...
        if(!fromRefresh)
            initDialogConfirm("Tampaknya kamu mempunyai beberapa tugas yang harus diselesaikan, jangan lupa dikerjakan ya", "Baik, akan saya kerjakan", R.drawable.ic_man_thinking, object : DialogConfirm.OnDialogConfirm{
                override fun onCancelConfirmClick() {
...
//            Toast.makeText(this, "Job Service is already scheduled", Toast.LENGTH_SHORT).show();
        ...
        val mServiceComponent = ComponentName(this, CheckIntervensiJobScheduler::class.java)
